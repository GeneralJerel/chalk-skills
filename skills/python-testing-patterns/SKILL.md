...
- **No assertion messages** — `assert result == expected` gives poor failure output. Use `assert result == expected, f"Expected {expected}, got {result}"` or rich assertion libraries like `pytest`'s built-in diffs.
